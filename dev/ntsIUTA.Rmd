...
author:
- name: Ricardo Cunha
  affiliation: Institut für Energie- und Umwelttechnik e. V. (IUTA)
package: ntsIUTA
abstract: >
...
`ntsIUTA` depends on external open source software. These external dependencies have to be installed beforehand using the instructions under Prerequisites.
...
The `ntsIUTA` package can be entirely loaded with the code line below but it is recommended to use `ntsIUTA::` before every function call for better interpretation of the script.

```{r pkgLoad}
library(ntsIUTA)
```

The basic back-end workflow requires an initial setup where MS data files are provided. The creation of a project folder containing the MS files of interest is highly recommended and can be used for setup. The function `setupProject` requires the path of the project folder for the argument `projectPath`. Then, the directory and any existing sub-folders are screened for MS files. The data format used in `ntsIUTA` is `mzML` containing centroided MS data. However, a wide variety of data formats, including vendor MS data, can be used as input. The MS files can be converted to `mzML` through `r Githubpkg("rickhelmus/patRoon")` and/or [ProteoWizard](http://proteowizard.sourceforge.net/) within the `setupProject` function. Conversion of MS files in project folder to `mzML` can be done by setting the argument `convertFiles` to `TRUE`. Currently, the MS file formats recognized are as follows: *thermo*, *bruker*, *agilent*}, *ab* (from *AB Sciex*) and *waters*. See `?setupProject` for more information. By setting `makeNewProject` to `TRUE`, a new R project will be created in the given project folder and opened in a new R session, when using RStudio. When not using RStudio, the working directory of the running R session will be changed to the given `projectPath`.

The output from `setupProject` is a simple [data.frame](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/data.frame) with the following columns:
...
* `date`, the project date which by default is the `Sys.Date` when the project is created;
* `wdir`, the project working directory to be used for cross project analysis. 

The code below loads the mzML files from the given projPath and selects the six first files containing centroided data to be exploited further in this document. Setting `save` to `TRUE`, the `sampleInfo` object is store in the rData folder created in the `projPath`. Most functions in `ntsIUTA` allow to store the objects in the rData folder for further use. For instance, in cross analysis single objects can be loaded without the need for rerunning the basic workflow of ntsIUTA. When `save` is set to `TRUE`, the `projPath` should be given. The `getwd()` is used by default and the created object is saved in the rData subfolder.

```{r setup}
projPath <- system.file(package = "ntsIUTA", dir = "extdata")
sampleInfo <- setupProject(projPath,
                           save = FALSE,
                           makeNewProject = FALSE)
#Filter for selecting the six first files in the sampleInfo
sampleInfo <- sampleInfo[1:6, ]

#below the 5 first columns of the sampleInfo
```

```{r sampleInfoExample, cache=FALSE, echo=FALSE, message=FALSE, results='axis'}
sampleInfo_temp <- sampleInfo[, 1:5]
sampleInfo_temp$filePath <- "C:/FilePath/FileName.mzML"
sampleInfo_temp %>% kbl(caption = "sampleInfo Example") %>% kable_paper("hover", full_width = FALSE) %>% kable_styling(position = "left")
```

# Importing data from mzML

After project setup, the data from the mzML files listed in the `sampleInfo` can be imported via `importRawData`. For importing, we used the `r Biocpkg("MSnbase")` which reads both centroided and profile MS data in mzML and mzXML files. The returning object is an [OnDiskMSnExp](https://rdrr.io/bioc/MSnbase/man/OnDiskMSnExp-class.html) object which can also be used for native functions within the `r Biocpkg("MSnbase")` and `r Biocpkg("xcms")` packages. In addition, `importRawData` can refine the time window of the files using the `rtFilter` and empty septra can be removed by setting `removeEmptySpectra` to `TRUE`. See `?importRawData` for additional information.
...
`rawData` can be inspected to improve downstream processing, such as peak finding. Iteractive total, base and extracted ion chromatograms (TIC, BPC and EIC, respectively) can be obtained via `plotRawChrom` as shown below, using the `r CRANpkg("plotly")` package. See `?plotRawChrom` for more details and arguments.

```{r plotTIC, cache=TRUE,echo=TRUE,message=FALSE,warning=FALSE,results='markup',fig.align="left",out.width = "70%",fig.cap ="TIC plot."}
plotRawChrom(rawData, type = "tic")
```

The `plotRawChrom` can be used for EIC by specifiying the `mz` and deviation (`ppm`), as shown below for diuron. When `mz` is specified, the type is automatically set to `eic` although `tic` is given as argument.
...
MS data can be directly centroided during conversion to mzML with [ProteoWizard](http://proteowizard.sourceforge.net/) via `setupProject`. However, centroiding of profile MS data was also implemented with additional capabilities for smotthing and refining the MS data, using tools from the `r Biocpkg("MSnbase")` package. The smoothing and refinement can benefit peak finding for data with high noise and low mass accuracy and/or high deviation. Below we plot the example for centroided and profile MS data of Terbutryn. A high deviation for the mass of Terbutryn is shown on the centroided plot (left), which is likely related to saturation of the MS detector.

```{r plotCentProf,cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='markup', out.width = "70%", fig.cap="Centroided vs Profile data. Comparison between centroided and profile data for Terbutryn as [M+H]+."}
sampleInfo_prof <- setupProject(projPath, save = F, makeNewProject = F)

sampleInfo_prof <- sampleInfo_prof[7:9, ]

rawData_prof <- importRawData(sampleInfo = sampleInfo_prof,
                              rtFilter = c(13, 17),
                              timeUnit = "min",
...
p1 <- plotTargetCentroids(rawData, fileIndex = 4,
                          mz = 242.1434, ppm = 90,
                          rt = 14.65, rtWindow = 0.5,
                          rtUnit = "min", plotTargetMark = TRUE)

p2 <- plotTargetCentroids(rawData_prof, fileIndex = 1,
                          mz = 242.1434, ppm = 90,
                          rt = 14.65, rtWindow = 0.5,
                          rtUnit = "min", plotTargetMark = TRUE)

subplot(list(p1, p2), nrows = 1, margin = 0.04)
```
...
rawData_prof2 <- centroidProfileData(filterFile(rawData_prof, 1),
                                     halfwindow = 3,
                                     SNR = 0,
                                     noiseMethod = "MAD",
...
p2 <- plotTargetCentroids(rawData_prof2, fileIndex = 1,
                          mz = 242.1434, ppm = 30,
                          rt = 14.65, rtWindow = 0.5,
                          rtUnit = "min",
                          plotTargetMark = TRUE)
...
peaksData_prof2 <- peakPicking(rawData_prof2,
                               param = param,
                               save = FALSE)
```
