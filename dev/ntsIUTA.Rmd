...
To obtained the EIC raw data the function `extractEIC` can be used, returning a data frame as shown below with the retention time, *m/z* and intensity for the specified `mz` and respective deviation (`ppm`). 
...
sampleInfo_prof <- ntsIUTA::setupProject(projPath, save = F, makeNewProject = F)

sampleInfo_prof <- sampleInfo_prof[7:9, ]

rawData_prof <- ntsIUTA::importRawData(sampleInfo = sampleInfo_prof,
                                       rtFilter = c(13, 17),
                                       timeUnit = "min",
...
rawData_prof2 <- ntsIUTA::centroidProfileData(MSnbase::filterFile(rawData_prof, 2),
                                              halfwindow = 3,
                                              SNR = 0,
                                              noiseMethod = "MAD",
