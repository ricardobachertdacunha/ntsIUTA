...
#'
checkAnnotation <- function(obj = NULL,
                            samples = NULL,
                            ID = NULL,
                            mz = NULL, ppm = 5,
...
#'
plotAnnotation <- function(obj = NULL,
                           samples = NULL,
                           ID = NULL,
                           mz = NULL, ppm = 5,
                           rt = NULL, rtWindow = 1, rtUnit = "sec",
                           ...
                           onlyRelated = TRUE,
                           log = TRUE,
                           colorBy = c("groups", "samples")) {
  
  checkmate::checkSubset(colorBy, c("groups", "samples"))
  
  checkmate::checkSubset(rtUnit, c("sec", "min"))
  
  ft <- checkAnnotation(obj = obj,
                        samples = samples,
                        ID = ID,
                        mz = mz, ppm = ppm,
  ...
  if (colorBy == "groups") {
    ...
    for (s in seq_len(length(spNames))) {
      plot <- plot %>% add_bars(x = ft$mz[ft$group %in% names(spNames)[s]],
                                y = ft[ft$group %in% names(spNames)[s], colnames(ft) %in% spNames[s], drop = T],
