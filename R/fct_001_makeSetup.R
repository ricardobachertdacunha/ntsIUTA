...
#' @param title A character string with the project title. The default is \code{NULL}.
#' @param description A character string with the project description. The default is \code{NULL}.
...
setupProject <- function(path = utils::choose.dir(getwd(), "Select or create a project folder"),
  ...
  if (!is.null(title)) obj@title <- title
  
  if (!is.null(description)) obj@info$description <- description
  ...
  obj@metadata <- obj@samples[,"sample", drop = F]
...
#' @title addFiles
#' @description Adds mzML or mzXML files to a \linkS4class{ntsData} object.
#'
#' @param newFiles A list of paths for selected mzML or mzXML files.
#' The default is a UI to choose the files.
#' @param obj A \linkS4class{ntsData} object to add the selected files.
#' @param copyFiles Logical. Set to \code{TRUE} to copied the selected files
#' to the \code{path} given in the \linkS4class{ntsData} object.
#' \code{FALSE} will add the original location of the files
#' to the \code{samples} table. Note that if duplicate names exist,
#' the copied files are renamed or when \code{copyFiles} is \code{FALSE},
#' the original files will be renamed.
...
#' the class method \code{setGroups}.
#'
...
#' Therefore, data processing should be rerun.
#'
...
addFiles <- function(newFiles = utils::choose.files(),
  ...
  if (TRUE %in% duplicates) {
    ...
    newJoint <- removeDuplicateNames(newJoint = oldJoint)
  } else {
    newJoint <- oldJoint
  }
  ...
  if (!(duplicateNamesPrompt) || is.na(duplicateNamesPrompt))  {
    ...
    newDuplicates <- !(oldJoint$sample == newJoint$sample)

    if (copyFiles) {
      mzML <- paste0(obj@path, "\\mzML")
...
saveObject <- function(format = "rds",
  ...
  #fdate <- obj@date
  #fdate <- gsub(pattern = "-", replacement = "", x = as.character(fdate))
