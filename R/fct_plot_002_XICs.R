...
#' @param plotTargetMark Logical, set to \code{TRUE} (the defaulft) to plot a
#' target mark.
...
#' Note that each target is always plotted in one row for all selected samples.
#'
  ...
  for (t in ids) {
    ...
    for (s in seq_len(length(xic_s))) {
      ...
      p1 <- p1 %>% add_annotations(
        text = paste(unique(temp$sample), t, sep = " - "),
